...
{ 
    ...
    void FixedUpdate()
    {
        ...
        if (dv.magnitude > speedDiff.magnitude)
            dv = dv.normalized * speedDiff.magnitude;

        Vector2 v0 = m_Velocity;

        m_Velocity = m_Velocity + dv;

        Vector2 dp = (v0 + m_Velocity) * Time.deltaTime * 0.5f;

        Vector2 framedAcceleration = new Vector2(groundNormal.y, -groundNormal.x) * dp.x;
        ...
        Vector2 yMove = Vector2.up * dp.y;// new Vector2(0, dp.y);
