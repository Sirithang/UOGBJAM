...
public class RoomManager : MonoBehaviour, ISerializationCallbackReceiver
{
    ...
    public int activeRoom { get { return _activeRoom; } }

    protected int _activeRoom = 0;

    protected Dictionary<int, List<RoomObject>> _roomObjects;
    ...
    private void Awake()
    {
        ...
        _roomObjects = new Dictionary<int, List<RoomObject>>();
    }
    ...
    public void SwitchRoomObject(int room, bool active)
    {
        if (_roomObjects.ContainsKey(room))
        {
            List<RoomObject> objList = _roomObjects[room];
    ...
    public void RegisterRoomObject(int room, RoomObject obj)
    {
        if (!_roomObjects.ContainsKey(room))
            _roomObjects[room] = new List<RoomObject>();

        _roomObjects[room].Add(obj);
    }

    public void UnregisterRoomObject(RoomObject obj)
    {
        if (!_roomObjects.ContainsKey(obj.room))
            return;

        _roomObjects[obj.room].Remove(obj);
    }
